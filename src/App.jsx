  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 transition-colors duration-500 dark:bg-slate-950 dark:text-slate-100">
      ...
      <div className="relative isolate overflow-hidden py-8 px-6 md:px-10 xl:px-16">
        ...
        <main className="mx-auto mt-10 max-w-7xl space-y-24">
          <section id="home" className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr] xl:items-center">
            <motion.div initial={{ opacity: 0, y: 36 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="space-y-8">
              <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm text-slate-200 shadow-glass backdrop-blur-xl dark:border-slate-700/60 dark:bg-slate-900/70">
              ...
              <div className="space-y-4">
                ...
                <h1 className="text-5xl font-semibold leading-tight tracking-tight md:text-6xl">Crafting modern web experiences with a premium developer mindset.</h1>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.9 }} className="relative flex flex-col items-center justify-center gap-6 rounded-[40px] border border-white/10 bg-white/10 p-6 shadow-glass backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-900/80">
              ...
              <div className="space-y-3 text-center">
                <p className="text-sm uppercase tracking-[0.35em] text-slate-400">Software Developer</p>
                <h2 className="text-3xl font-semibold">{personal.name}</h2>
                <p className="text-base text-slate-400">{personal.education}</p>
              </div>
              <div className="w-full rounded-3xl border border-white/10 bg-slate-950/70 p-5 text-center text-sm text-slate-300 shadow-inner shadow-black/20">
                <p className="text-slate-200">{typedText}<span className="animate-pulse text-cyan-400">|</span></p>
              </div>
            </motion.div>
          </section>

          <section id="about" className="grid gap-10 lg:grid-cols-[0.9fr_0.6fr]">
            <div className="space-y-8 rounded-[32px] border border-white/10 bg-white/10 p-8 shadow-glass backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-900/80">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="rounded-3xl border border-white/10 bg-slate-950/80 p-5 text-slate-300">
                  <p className="text-sm uppercase tracking-[0.35em] text-slate-400">College</p>
                  <p className="mt-2 text-lg font-semibold">Jawaharlal College of Engineering and Technology</p>
                </div>
                <div className="rounded-3xl border border-white/10 bg-slate-950/80 p-5 text-slate-300">
                  <p className="text-sm uppercase tracking-[0.35em] text-slate-400">Contact</p>
                  <p className="mt-2 text-lg font-semibold">{personal.email}</p>
                </div>
              </div>
            </div>
            <div className="space-y-6 rounded-[32px] border border-white/10 bg-white/10 p-6 sm:p-8 shadow-glass backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-900/80">
              <div className="flex items-center gap-3 text-slate-300">
                ...
                <div>
                  ...
                  <h3 className="text-xl font-semibold">{personal.education}</h3>
                </div>
              </div>
              <div className="space-y-4 rounded-3xl border border-slate-700/50 bg-slate-950/80 p-6">
                {[
                  { name: 'React', level: 90 },
                  { name: 'JavaScript', level: 88 },
                  { name: 'Python', level: 86 },
                  ...
                  <div key={skill.name} className="flex items-center justify-between gap-3 text-slate-300">
                    <span>{skill.name}</span>
                    <span className="text-sm text-slate-400">{skill.level}%</span>
                  </div>
                ))}
              </div>
              <div className="rounded-3xl bg-gradient-to-br from-violet-500/20 via-cyan-500/10 to-slate-900/40 p-6 text-slate-100">
                <p className="text-sm uppercase tracking-[0.35em] text-cyan-200">Future Focus</p>
                <p className="mt-3 text-lg font-semibold">Building creative, accessible, and scalable digital products as a recruiter-ready software engineer.</p>
              </div>
            </div>
          </section>

          <section id="skills" className="space-y-10">
            <div className="rounded-[36px] border border-white/10 bg-white/10 p-8 shadow-glass backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-900/80">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm uppercase tracking-[0.35em] text-cyan-300">Skills</p>
                  <h2 className="text-3xl font-semibold">Technical expertise</h2>
                </div>
              ...
              <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {skills.map((skill) => (
                  <motion.div key={skill.category} whileHover={{ y: -6 }} className="rounded-3xl border border-white/10 bg-slate-950/80 p-6 shadow-lg shadow-slate-950/25 transition">
                    <p className="text-sm uppercase tracking-[0.35em] text-cyan-300">{skill.category}</p>
            ...
            <div className="rounded-[36px] border border-white/10 bg-white/10 p-8 shadow-glass backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-900/80">
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <h2 className="text-3xl font-semibold">Highlighted work</h2>
                </div>
                <div className="inline-flex rounded-full border border-white/10 bg-slate-950/80 px-4 py-2 text-sm text-slate-300">
                  <FaGithub className="mr-2 h-4 w-4" /> GitHub-ready showcase
                </div>
              </div>
              <div className="mt-10 grid gap-6 xl:grid-cols-3">
                {filteredProjects.map((project) => (
                  <motion.article key={project.name} whileHover={{ y: -8 }} className="group overflow-hidden rounded-[32px] border border-white/10 bg-slate-950/80 p-6 shadow-glass backdrop-blur-xl transition">
                    <div className="relative mb-6 h-56 overflow-hidden rounded-3xl bg-gradient-to-br from-slate-800 via-slate-900 to-slate-950">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center text-7xl text-slate-600 opacity-20">{project.name[0]}</div>
                    </div>
                    <span className="text-sm uppercase tracking-[0.35em] text-cyan-300">{project.name}</span>
                    <p className="mt-4 text-slate-300">{project.description}</p>
                    <div className="mt-6 flex flex-wrap gap-2">
                      {project.technologies.map((tech) => (
                        <span key={tech} className="rounded-full border border-slate-700/50 bg-slate-900/70 px-3 py-1 text-xs text-slate-300">{tech}</span>
                      ))}
                    </div>
                    <div className="mt-8 flex flex-wrap gap-3">
                      ...
                      <a href={project.live} className="inline-flex items-center gap-2 rounded-full border border-slate-700/60 px-4 py-3 text-sm font-semibold text-slate-100 transition hover:border-slate-500" target="_blank" rel="noreferrer">
          ...
          <section id="education" className="grid gap-10 lg:grid-cols-[0.7fr_0.3fr]">
            <div className="rounded-[36px] border border-white/10 bg-white/10 p-8 shadow-glass backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-900/80">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm uppercase tracking-[0.35em] text-cyan-300">Education</p>
                  <h2 className="text-3xl font-semibold">Academic timeline</h2>
                </div>
              </div>
              <div className="mt-10 space-y-8">
                <div className="relative pl-8 before:absolute before:left-2 before:top-2 before:h-full before:w-px before:bg-slate-500/30">
                  <div className="absolute left-0 top-2 h-4 w-4 rounded-full bg-cyan-400" />
                  <p className="text-sm uppercase tracking-[0.35em] text-slate-400">2023 - 2027</p>
                  <h3 className="mt-2 text-2xl font-semibold">B.Tech Computer Science & Data Science</h3>
                  <p className="mt-2 text-slate-300">Jawaharlal College of Engineering and Technology</p>
                </div>
              </div>
            </div>
            <div className="rounded-[36px] border border-white/10 bg-white/10 p-8 shadow-glass backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-900/80">
              ...
              <div className="mt-8 grid gap-4">
                {stats.map((item) => (
                  <motion.div key={item.label} whileHover={{ scale: 1.02 }} className="rounded-3xl bg-slate-950/80 p-6 text-center">
                    <p className="text-4xl font-semibold text-cyan-300">{item.value}+</p>
                    <p className="mt-2 text-sm uppercase tracking-[0.35em] text-slate-400">{item.label}</p>
                  </motion.div>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-[36px] border border-white/10 bg-white/10 p-8 shadow-glass backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-900/80">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h2 className="text-3xl font-semibold">Milestones & certifications</h2>
                </div>
              </div>
              <div className="mt-10 grid gap-4 md:grid-cols-3">
                {achievements.map((item) => (
                  <motion.div key={item} whileHover={{ y: -6 }} className="rounded-3xl border border-slate-700/50 bg-slate-950/80 p-6 text-slate-300">
                    <p>{item}</p>
                  </motion.div>
                ))}
              </div>
          ...
          <section id="contact" className="grid gap-10 lg:grid-cols-[0.9fr_0.6fr]">
            <div className="rounded-[36px] border border-white/10 bg-white/10 p-8 shadow-glass backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-900/80">
              ...
              <form className="mt-10 space-y-6 rounded-3xl border border-slate-700/50 bg-slate-950/80 p-6">
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="space-y-2 text-slate-300">
                    <span className="text-sm font-medium">Name</span>
                    <input type="text" placeholder="Your name" required className="w-full rounded-3xl border border-slate-700/60 bg-slate-900 px-4 py-3 text-sm text-slate-100 outline-none transition focus:border-cyan-400" />
                  </label>
                  <label className="space-y-2 text-slate-300">
                    <span className="text-sm font-medium">Email</span>
                    <input type="email" placeholder="you@example.com" required className="w-full rounded-3xl border border-slate-700/60 bg-slate-900 px-4 py-3 text-sm text-slate-100 outline-none transition focus:border-cyan-400" />
                  </label>
                </div>
                <label className="space-y-2 text-slate-300">
                  <span className="text-sm font-medium">Message</span>
                  <textarea rows="5" placeholder="Tell me about your project" required className="w-full rounded-3xl border border-slate-700/60 bg-slate-900 px-4 py-3 text-sm text-slate-100 outline-none transition focus:border-cyan-400" />
                </label>
              ...
              <div className="space-y-4 rounded-3xl bg-slate-950/80 p-6 text-slate-300">
                ...
                <div className="space-y-4 text-sm">
                  <div>
                    ...
                    <a href={`mailto:${personal.email}`} className="text-cyan-300">{personal.email}</a>
                  </div>
                  <div>
                    <p className="text-slate-400">LinkedIn</p>
                    <a href={personal.linkedIn} target="_blank" rel="noreferrer" className="text-cyan-300">linkedin.com/in/anand-tk-770719287</a>
                  </div>
                  <div>
        ...
        <footer className="mt-20 rounded-[32px] border border-white/10 bg-white/10 p-8 text-center text-slate-400 shadow-glass backdrop-blur-xl dark:border-slate-700/50 dark:bg-slate-900/80">
